...
측정 정의 (B의 마네킹 `airis/sim/body.py`와 같은 의미가 되게 맞췄다)
- 키 = 머리 꼭대기 − 바닥. 코(없으면 눈·귀)와 발목 키포인트 사이 거리에 아래 인체 비율
  보정(머리 꼭대기→코, 발목 키포인트→바닥)을 더한다.
- 어깨 너비 = 좌우 어깨 키포인트 거리 (B: 어깨 관절 간격).
- 팔 길이 = 어깨–팔꿈치 + 팔꿈치–손목 (B: 상완 + 전완).
- 다리 길이 = 엉덩이–무릎 + 무릎–발목 + 발목 높이 (B: 골반 관절 높이, 발목 관절이 바닥).
- 몸통 두께는 정면으로 못 재므로 `TORSO_DEPTH_PER_HEIGHT × 키` (C 데이터셋 샘플러와 같은 비율).
...
아래 인체 비율 상수는 인체 측정 평균에서 잡은 추정값이다 (실측 사진으로 보정 필요, PR 본문).
합성 키포인트(`synthetic_keypoints`)도 같은 상수로 B 관절에 코·눈·귀·발목 키포인트를 붙인다.
"""
...
def _head_reference(kp: np.ndarray, ok: np.ndarray) -> tuple[np.ndarray, float] | None:
    """(머리 기준점 px, 그 점의 머리 꼭대기 아래 비율). 코 → 두 눈 평균 → 두 귀 평균 순."""
    if ok[NOSE]:
        return kp[NOSE], NOSE_BELOW_TOP_PER_HEIGHT
    for pair, below in (((L_EYE, R_EYE), EYE_BELOW_TOP_PER_HEIGHT),
                        ((L_EAR, R_EAR), EAR_BELOW_TOP_PER_HEIGHT)):
...
def estimate_body(kp: np.ndarray, conf: np.ndarray, scale_m_per_px: float | None = None, *,
                  ...
                  seated: bool = False, min_conf: float = MIN_CONF) -> BodyEstimate:
    ...
    if seated:
        ...
        span_ratio = 1.0 - head_below - SEATED_LEG_PER_HEIGHT     # 코 ↔ 골반 관절 / 키
        scale = height_m * span_ratio / span_px
        height = float(height_m)
        leg = SEATED_LEG_PER_HEIGHT * height
        ...
        span_ratio = 1.0 - head_below - ANKLE_ABOVE_FLOOR_PER_HEIGHT   # 머리 기준점 ↔ 발목 / 키
...
def check_plausible(body: BodyParams) -> list[str]:
    """범위를 벗어난 필드 이름 목록. B의 몸통 길이 조건(다리 < 키 × 0.83)도 본다."""
    ...
    if body.leg_length_m >= body.height_m * (1.0 - 2 * HEAD_RADIUS_PER_HEIGHT - 0.04):
...
# ---------------------------------------------------------------------------
# 합성 키포인트 (테스트·데모용): B 관절을 정면 핀홀 카메라에 투영
# ---------------------------------------------------------------------------
...
def synthetic_keypoints(body: BodyParams, pose: PoseParams, scenario: Scenario,
    ...
    kp = camera.project(kp3, center_x=float(joints["pelvis"][0]))
    if noise_px > 0:
