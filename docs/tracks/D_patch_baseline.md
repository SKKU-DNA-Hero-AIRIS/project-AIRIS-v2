...
## 모델 한계 (PR #11·#18·#26·#30·#31·#58·#60 발견 사항, 2026-09-18 갱신)
...
4. **슬롯 노즐 가림은 슬롯 축 위 K점의 보이는 비율로 근사한다** (PR #30). 길이 0.6 m 슬롯을 중심 한 점으로 보면 팔 하나가 슬롯 전체를 가린 것으로 판정돼 순위까지 달라졌다(K=9 대비 Spearman 0.963). 기본 K=3은 K=9 대비 Spearman 0.994, 총 R 최대 상대 차 8.9%, 검증용 K=5는 0.998, 5.2%다 (무작위 자세 60개, 400/m², 보정 켬). 슬롯 점마다 제트 세기를 다르게 두지 않고, 보이는 비율을 그 노즐의 전체 기여에 곱한다. 부스 안 무작위 자세 200개로 다시 재면 K=3의 K=9 대비 최대 상대 차가 캡슐 9.2%, 메시 11.6%다(p99 8.8%·8.3%, Spearman 둘 다 0.995). 팔 45~60°·yaw 30° 부근이 메시에서 10~12%로 가장 커서, 테스트 허용치를 K=3 15%, K=5 5%로 둔다 (PR #58).
...
7. **메시 몸의 겨드랑이·옆구리 제거율이 캡슐보다 훨씬 작다** (슬롯 바, 보정 켬, 2000/m², `MESH_DEFAULT_BODY`). 팔 150°에서 옆구리 R은 캡슐 0.078, 메시 0.0049다(20°에서는 0.019 / 0.0023). 방향(팔을 들면 상승)은 두 몸 모두 맞다. 옆구리가 슬롯에서 보이는 비율(캡슐 0.32, 메시 0.49)과 풍속(중앙값 3.7 / 3.2 m/s)은 비슷하므로 가림·슬롯 높이 문제가 아니다. 메시 옆구리 면은 옆 슬롯을 더 정면으로 보고 조금 아래를 향한다(법선 z 평균 캡슐 +0.11, 메시 −0.36). 그래서 접선 속도 중앙값이 3.4에서 2.2 m/s로 줄고, 전단은 제곱이라 0.034에서 0.014 Pa가 된다. τ_med 0.125 Pa보다 훨씬 낮은 영역이라 로그 정규 CDF가 가팔라 제거율 차이가 약 13배로 커진다. 팔 부위 면적(캡슐 0.33 m², 메시 0.29~0.30 m²)은 원인이 아니다.
8. **휠체어에서 캡슐과 메시의 순위 일치가 낮다** (E5 score ρ 0.767, total 0.514). 캡슐은 자세에 따른 총 R 차이가 작고(표준편차 0.0034, 메시 0.0072), 상체 숙임(torso_pitch)의 효과 방향이 두 몸에서 반대다(weighted_removal과의 Spearman 캡슐 +0.14, 메시 −0.28). 메시에서는 숙이면 앉은 허벅지가 가려져 다리 R이 크게 떨어진다(메시 다리 R 평균 0.086, 캡슐 0.047).
...
## 단계 8. 메시 가림과 E5 (③, 2026-09-18 총괄 확정, `docs/mesh_transition.md`)

1. `occlusion`: `state.mesh_vertices`가 있으면 Open3D `RaycastingScene`(`pip install open3d`, 사용자 승인) 광선-삼각형 교차로 가시 비율을 계산. 원점은 노즐 가림점(슬롯 K=3 유지), 대상은 패치, `patch_face` 자기 면 제외. 캡슐 경로는 폴백으로 유지. B의 sim 메시가 오기 전에는 `tests/fakes.py`의 가짜 메시(예: 정육면체+원기둥)로 시작.
2. 성능: 400/m² 1회 중앙값 100 ms 이하(저부하). 장면 생성이 병목이면 후보당 1회만 만든다.
3. `scripts/compare_bodies.py`(E5): 시나리오별 부스 안 자세 300개를 캡슐판·메시판으로 채점해 Spearman(score·부위별)과 제거율 범위를 `docs/experiments.md`에 기록(C와 협의). 통과 기준 없음.
4. `tests/test_sanity_physics.py`의 E1은 메시 모델에서도 성립해야 한다(정면·측면·세기 단조).

## E5 결과: 캡슐 몸 vs 메시 몸 (2026-09-18, main c163eff + #60)

시나리오별로 두 몸 모두 부스 안인 자세 300개를 균등 샘플링(seed 0)했다. 체형은 두 몸 모두 `MESH_DEFAULT_BODY`(1.70/0.342/0.194/0.463/0.883)이고, 패치판 2000/m², 슬롯 가림 K=3, 퓨리움 슬롯 바 12개, 충돌 보정 켬, 상수는 `configs/physics.yaml` 그대로다. 명령은 `python scripts/compare_bodies.py --n 300`이다. 통과 기준은 없다(차이 기록).

| Spearman ρ | default | pregnant | wheelchair |
|---|---|---|---|
...
총 제거율 평균(범위): default 캡슐 0.107(0.061~0.158) / 메시 0.125(0.073~0.183), pregnant 0.113 / 0.130, wheelchair 0.054(0.037~0.063) / 0.073(0.048~0.090).

- 메시가 총 R이 1.7~1.9%p 높다. 다리가 가장 크게 오르고(+0.040~0.049), 몸통 뒤는 내린다(−0.017~−0.022). 메시 다리는 두 다리가 떨어져 있어(고관절 간격 0.23 m, 캡슐 0.08 m) 안쪽 면도 바람을 받는 것으로 보인다(확인하지 않은 추정).
- 서 있는 시나리오(default·pregnant)는 score 순위가 0.93~0.95로 대체로 유지된다. 휠체어는 0.77로 낮다(모델 한계 8번).
- 부스 밖으로 버린 자세: default 38, pregnant 13, wheelchair 30. 채점 시간(300개, 2000/m²): 캡슐 29~41 s, 메시 16~23 s.
- 파일: `outputs/e5/summary.md`, `outputs/e5/<scenario>/`(samples.csv, summary.json, scatter.png, top_mismatch.md).
